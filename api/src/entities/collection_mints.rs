...
use sea_orm::{entity::prelude::*, JoinType, QuerySelect, SelectTwo, Set};
...
impl ActiveModelBehavior for ActiveModel {
    ...
    fn before_save(mut self, _insert: bool) -> Result<Self, DbErr> {
        if self.owner.as_ref().starts_with("0x") {
            self.owner = Set(self.owner.as_ref().to_lowercase());
        }

        if let Some(a) = self.address.as_ref() {
            if a.starts_with("0x") {
                self.address = Set(Some(a.to_lowercase()));
            }
        }
